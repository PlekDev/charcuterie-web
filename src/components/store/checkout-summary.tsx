...
export function CheckoutSummary({ notes = '' }: CheckoutSummaryProps) {
  ...
  return (
    <Card className="rounded-2xl border-black/5 shadow-xl bg-white overflow-hidden sticky top-32">
      <CardHeader className="p-8 border-b bg-surface/20">
         <div className="flex items-center gap-2 mb-2">
            <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-secondary">Resumen del pedido</span>
         </div>
         <CardTitle className="text-2xl font-serif font-bold text-on-surface">Tu Compra</CardTitle>
      </CardHeader>
            ...
            {items.map(item => (
               <div key={item.id} className="flex items-center justify-between text-sm">
                  <div className="flex items-center gap-3">
                     <span className="w-5 h-5 rounded-full bg-surface flex items-center justify-center text-[10px] font-bold text-on-surface border border-black/5">
                        {item.quantity}
                     </span>
                     <span className="text-on-surface font-medium line-clamp-1">{item.name}</span>
                  </div>
         ...
         <div className="flex flex-col gap-3 pt-6 border-t border-black/5">
            ...
            <div className="flex justify-between text-sm">
               <span className="text-muted-foreground flex items-center gap-2">
                  Envío
                  {shipping === 0 && (
                     <span className="text-[9px] font-bold uppercase tracking-widest text-primary bg-primary/10 px-2 py-0.5 rounded-full">Gratis</span>
                  )}
               </span>
               <span className={shipping === 0 ? "text-primary font-bold" : "text-on-surface font-bold"}>
